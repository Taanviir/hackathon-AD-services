    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if new_employee:
                ...
                response.set_cookie(
                    "access_token", str(refresh.access_token), samesite='None', httponly=True
                )
                response.set_cookie("refresh_token", str(refresh), samesite='None', httponly=True)
                print(f"access tokens: {str(refresh.access_token)}, =>  Refresh: {str(refresh)}", flush=True)
                return response
            return Response(
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if employee:
                ...
                response.set_cookie('access_token', str(refresh.access_token), samesite='None', httponly=True)
                response.set_cookie('refresh_token', str(refresh), samesite='None', httponly=True)
